...
func run() error {
	walker := flag.String("walker", "armham", "retail walking unit")
	heading := flag.Int("heading", 24000, "metal initial heading")
	out := flag.String("out", "/private/tmp/nanolathe-feature-v3-trails-media/probe", "output")
	mapname := flag.String("map", "Comet Catcher", "retail map")
	scene := flag.String("scene", "metal", "ground trails")
	...
	// Clear only the authored demonstration lanes before placing the two units.
	for z := (center.Z - 140) / 16; z <= (center.Z+140)/16; z++ {
		for x := (center.X - 210) / 16; x <= (center.X+210)/16; x++ {
			s.Features.RemoveFeatureAt(int(x), int(z), features.CauseBurnt)
		}
	}
	fmt.Printf("scene=%s center=%+v poses=%+v sea=%d\n", *scene, center, poses, s.World.SeaLevel)
	names := []string{"armbats", "corcrus"}
	if *scene == "metal" {
		names = []string{*walker, "armbull"}
	}
	...
	for i, h := range g.handles {
		...
		if *scene == "metal" {
			dest = pt{center.X + 130, int32(u.Z.Int())}
		}
...
func (g *game) capture() {
	...
	for _, v := range f.Units {
		if v.Slot == g.handles[0] {
			b, _ := json.Marshal(v.Pieces)
			row["walker_pose"] = json.RawMessage(b)
		}
	}
	...
	stats := &trailStats{}
	list.Replay(stats)
	row["footprints"] = stats.Feet
	row["track_segments"] = stats.Tracks
	g.r.SetDisplayPalette(g.c.DisplayPalette())
	g.r.SetGlow(false)
	g.r.SetBattleLighting(true)
	g.r.SetBlastDistortion(false)
	...
	for z := start; z < t.PlayBottom-512; z += 32 {
		for x := start; x < t.PlayRight-512; x += 32 {
			if scene == "metal" {
				...
				if ok {
					return pt{x, z}, [2]pt{{x - 140, z - 60}, {x - 140, z + 60}}, nil
				}
				continue
			}
...
type trailStats struct{ Feet, Tracks int }

func (s *trailStats) Trails(t drawlist.Trails) {
	for _, m := range t.Marks {
		if m.Shape == drawlist.TrailFootprint {
			s.Feet++
		} else {
			s.Tracks++
		}
...
func (s *trailStats) Clear()                   {}
func (s *trailStats) Expand()                  {}
func (s *trailStats) Terrain(drawlist.Terrain) {}
func (s *trailStats) Sprite(drawlist.Sprite)   {}
func (s *trailStats) Glyphs(drawlist.Glyphs)   {}
func (s *trailStats) Fill(drawlist.Fill)       {}
func (s *trailStats) Line(drawlist.Line)       {}
func (s *trailStats) Points(drawlist.Points)   {}
func (s *trailStats) Flash(drawlist.Flash)     {}
func (s *trailStats) Halo(drawlist.Halo)       {}
func (s *trailStats) Model(drawlist.Model)     {}
func (s *trailStats) Fog(drawlist.Fog)         {}
func (s *trailStats) Surface(drawlist.Surface) {}
func (s *trailStats) Cursor(drawlist.Cursor)   {}
