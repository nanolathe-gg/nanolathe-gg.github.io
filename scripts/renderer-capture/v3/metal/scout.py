...
import json,sys
root=Path(sys.argv[1] if len(sys.argv)>1 else '/private/tmp/nanolathe-feature-v3-metal-media/scout')
rows=[]
...
for row,(folder,best,_) in enumerate(rows):
 ...
 draw.text((8,row*200+5),f'{folder.name} heading {best["frame"]*4096}  OFF / ON  strong={best["strong"]}',fill='white')
