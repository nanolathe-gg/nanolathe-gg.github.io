...
func run() error {
	...
	out := flag.String("out", "/private/tmp/nanolathe-feature-v3-metal-media/scout", "output")
...
func (g *game) capture() {
	...
	g.meta = append(g.meta, map[string]any{"model": u.DefName, "frame": n, "heading": u.Heading, "width": width, "height": height, "zoom": 2, "model_stats": g.gpu.ModelStats()})
	if n%30 == 0 {
		fmt.Printf("model=%s frame=%d heading=%d stats=%+v\n", u.DefName, n, u.Heading, g.gpu.ModelStats())
	}
	g.index++
}
