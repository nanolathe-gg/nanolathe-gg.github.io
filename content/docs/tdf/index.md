+++
title = 'Text definitions & schemas'
seoTitle = 'TDF file format — illustrated Total Annihilation reference'
description = 'Explore TDF syntax, watch duplicate keys resolve, compare absent and empty values, and navigate the weapon, feature and global-data schemas.'
type = 'format'
url = '/docs/formats/tdf/'
format = 'TDF'
extension = '.tdf'
...
[[facts]]
label = 'Encoding'
value = 'Text, sampled ASCII / CP1252'
[[facts]]
label = 'Structure'
value = 'Nested named sections'
[[facts]]
label = 'Values'
value = 'Semicolon-terminated text'
[[facts]]
label = 'Lookup'
value = 'ASCII case-insensitive'
+++

## One grammar, many kinds of game data {#overview}
...
| Read the text | Build a tree | Compile a family |
| --- | --- | --- |
...
{{< callout kind="note" title="This is a syntax reference and a schema directory" >}}
The interactive examples and code snippets below are original authored illustrations. They show established parser rules or abbreviated schemas, and are not installable content packs. A TDF has no universal binary header: the tables describe grammar, keys and conversion boundaries, not fictitious file-byte offsets.
{{< /callout >}}

### Find the right schema {#schema-map}

| File family | Top-level records | What the consumer creates |
| --- | --- | --- |
| [FBI]({{< format-link "fbi" >}}) · `units/` | `[UNITINFO]` | Unit definitions and references to models, scripts, weapons and movement classes. |
| [GUI]({{< format-link "gui" >}}) · `guis/` | Ordered gadgets with nested `[COMMON]` | Panels, controls and artwork bindings. |
| [OTA]({{< format-link "ota" >}}) · `maps/` | Map/mission metadata | World settings, mission and placement information. |
| [`gamedata/`](#schema-families) | Family-specific names such as `[CLASS0]`, `[SIDE0]` | Movement, sides, sound categories, LOS and other global tables. |
| [`features/`](#features) | Feature names | Map objects, wrecks, reclaim yields and successor features. |
| [`weapons/`](#weapons) | Weapon names and nested `[DAMAGE]` | Weapon records selected by numeric ID. |
| [`download/`](#download) | Ordered placement sections | Builder/page/button placements. |
| [`camps/useonly/`](#useonly) | Unit-name sections, normally empty | Mission eligibility restrictions. |

{{< callout kind="warning" title="A newline does not end an assignment" >}}
`a=1` followed by `b=2;` is parsed as one value containing the newline and `b=2`, rather than two assignments. There are no quotes or escapes to protect a semicolon, and comments are blanked even inside would-be values. Use `}` to close a section; the extra semicolon in `};` is a Nanolathe extension, not traced retail grammar.
{{< /callout >}}

## Syntax and parsing {#syntax}
...
{{< format-demo name="tdf-duplicates" id="duplicate-example" title="The same spelling can survive twice" label="Watch the key store" >}}
Step through three assignments from the [original TDF](example.tdf). New case variants insert at the head of the equal-key run. When `Power` returns after `power`, its old spelling is behind the head, so it inserts again. Case-insensitive lookup reads the latest value, 9. The [checked state manifest](example.json) records every intermediate vector.
{{< /format-demo >}}
...
### Typed values and defaults {#typed-values}

{{< format-demo name="tdf-values" id="typed-example" title="Missing, empty and zero are different inputs" label="Follow the conversion" >}}
...
(Original abbreviated schema example; resource names are illustrative.) **Established:** the
...
### `features/` — map features and corpses {#features}
...
`_heap`). Files group many `[featurename]` sections. Original abbreviated example of a corpse feature, referenced by `Corpse=ex_relay_dead;`:
...
### `download/` — build-menu placement {#download}

Add-on units (and expansion units) attach themselves to construction menus
with `download/<unit>.tdf`. Original placement example:
...
### `camps/useonly/` — mission unit restrictions {#useonly}
...
- `maps/*.ota` — [ota.md]({{< format-link "ota" >}})
- `units/*.fbi` — [fbi.md]({{< format-link "fbi" >}})
- `guis/*.gui` — [gui.md]({{< format-link "gui" >}})
...
## Malformed text and loader outcomes {#malformed-text}
...
## Nanolathe implementation and safety policy {#host-policy}
