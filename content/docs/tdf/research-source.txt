...
| `MaxWaterSlope` | 3 | Slope limit that applies over water, separately from `MaxSlope`. `TANKDH3` authors `30`; both hover classes author `255` alongside `MaxSlope=12`, which is what lets a hovercraft cross steep sea floor. **Initialization:** every class record is pre-filled at startup with `MaxSlope` = `BadSlope` = `MaxWaterSlope` = `BadWaterSlope` = 255 and `MaxWaterDepth`/`MinWaterDepth` = ±10000 before any parse, and the engine then runs three unconditional clamps (`MaxSlope = min(MaxSlope, MaxWaterSlope)`, then the two Bad values clamped to their Max counterparts). The pre-fill supplies the absent-key default only for the two `Max*` slope keys and the two water-depth keys: an omitted `BadSlope` or `BadWaterSlope` instead defaults to half (`>>1`, logical) of the `MaxSlope` or `MaxWaterSlope` value read immediately before it, so the pre-filled 255 never survives an omitted `Bad*` key `[04 §6.1 R-DOC04-A]`. An omitted `MaxWaterSlope` therefore leaves `MaxSlope` at its authored value; the pool does **not** start zero-filled, so an omitted class does not compile to `MaxSlope = 0` `[04 §6.1 R-DOC04-A]`. |
