+++
title = 'Recordings, packets & reconstructed time'
seoTitle = 'TAD file format — illustrated Total Annihilation reference'
description = 'Peel apart a community match recording, restore Smartpak updates, and learn which bytes describe state, which describe capture, and which still need context.'
type = 'format'
url = '/docs/formats/tad/'
format = 'TAD'
extension = '.tad'
sourcePath = 'research/formats/tad.md'
sourceRevision = '72dcc024de8e6abb3b2f83137a292f566f13b63f'
demoScript = 'js/formats/tad.js'
demoCSS = 'css/tad.css'
[[facts]]
label = 'Producer'
value = 'TA Demo Recorder'
[[facts]]
label = 'Record prefix'
value = 'u16 total length'
[[facts]]
label = 'Observed file version'
value = '5 · nine-file corpus'
[[facts]]
label = 'Semantics'
...
## A recording of exchanged state {#overview}
...
{{< callout kind="established" title="A community recording format" >}}
The retail executable neither reads nor writes TAD. The wrapper and Smartpak transformations belong to third-party recorders; underlying retail network behavior has separate evidence in doc 08. Nanolathe does not contain a TAD parser, recorder or replay engine.
{{< /callout >}}
...
## Unpack a recorded update {#smartpak-example}

{{< format-demo name="tad-smartpak" id="smartpak-demo" title="Keep the state. Reconstruct the tick." label="Step through Smartpak" >}}
An original three-token fragment: set tick **600**, reconstruct one ground update, then expand an idle update at **601**. Download the [20-byte Smartpak stream](smartpak.bin), its [26-byte match-record wrapper](match-record.bin), or the [29 reconstructed bytes](reconstructed.bin). These are isolated teaching fragments, **not a complete TAD recording**.
{{< /format-demo >}}

{{< callout kind="note" title="The example supplies its own decoding context" >}}
This authored context has **283 retained definition rows including row zero**, so `W=9`; definition 1 is a ground unit. Its update sends local slot 0 and one path point `(120, 80)`, then an empty scheduled slot. The idle expansion follows the inspected recorder’s width-dependent convention. The fragment contains no player names, addresses, chat or private recording bytes. `scripts/tad-example.py --check` reproduces the data, independently walks its token framing and checks its movement bits.
{{< /callout >}}

## File, record, packet {#structure}
...
### Match-record wrapper

For version 5, match-record offsets also include the two-byte record prefix. The ordinary stored stream has already lost the custom wire checksum, mask and transport-sequence fields.

| Offset | Bytes | Type | Field | Meaning |
| --- | --- | --- | --- | --- |
| `+0` | 2 | `u16` | `totalLength` | This entire record, including the two length bytes. |
| `+2` | 2 | `u16` | `dtMs` | Recorder wall-clock milliseconds since the previous record. |
| `+4` | 1 | `u8` | `sender` | Explicit recorder participant number; join through setup records. |
| `+5` | 1 | `u8` | `flag` | `03` plain subpacket stream; `04` LZ77-compressed stream. |
| `+6` | variable | bytes | `payload` | Subpackets, including recorder Smartpak forms. |

The original example begins `1A 00 21 00 01 03`: **26 total bytes**, **33 ms**, sender **1**, plain stream. Its following 20 bytes are the Smartpak tokens in the demonstration above.

{{< callout kind="warning" title="Each length belongs to a different layer" >}}
The outer record length includes its two-byte prefix. A normal `2C` length includes its opcode, length and tick. A Smartpak `FD` preserves that original length while occupying **length − 4** stored bytes. Reading `FD` as an ordinary length-prefixed record shifts every following boundary.
{{< /callout >}}

### 2. Header record

Offsets below start at the header record’s **length prefix**, not at its body. This table describes version 5.

| Offset | Bytes | Type | Field | Meaning |
| --- | --- | --- | --- | --- |
| `+0` | 2 | `u16` | `length` | Total record bytes, including this prefix. |
| `+2` | 8 | `char[8]` | `magic` | `TA Demo` followed by NUL. |
| `+10` | 2 | `u16` | `version` | File format version. The observed corpus uses 5. |
| `+12` | 1 | `u8` | `numPlayers` | Number of player records and status records to follow. |
| `+13` | 2 | `u16` | `maxUnits` | Per-player unit-slot capacity, not a unit-definition count. |
| `+15` | variable | text | `mapName` | Map name, described as NUL-terminated in the owning format contract. |

| File version | Associated recorder generation | Header difference |
| --- | --- | --- |
| 3 | TADR 0.80b | `maxUnits` absent; map name starts at +13. Match packets also have four additional bytes after their flag. |
| 4 | TADR 0.81a | `maxUnits` absent; map name starts at +13. Do not assume the version-3 packet addition applies. |
| 5 | TADR 0.90b and later, through the observed 2020 recordings | Includes `maxUnits`, extra-sector count and extra-sector records. |
...
{{< callout kind="warning" title="Similar LZ77 tags do not imply the HPI window" >}}
Here the match offset is a **one-based absolute index into already produced output**, not a backward distance and not HPI’s 4096-byte ring-buffer position. Status records retain a custom encrypted envelope; stored match bodies do not.
{{< /callout >}}
...
{{< callout kind="unknown" title="A packet ID does not select one universal historical layout" >}}
Retail and recorder tables disagree on several sizes: notably `20` is 186 bytes in the inspected retail declaration and 192 in the observed recordings. The source explains which writer and reader establish each claim; the larger forms’ origin remains unresolved. Keep recorder/version evidence with the selected length table.
{{< /callout >}}
...
{{< format-demo name="tad-catalog" id="catalog-width-demo" title="The catalog determines the bit width" label="Explore bit packing" >}}
Compare **255** rows with **256**: the width jumps from 8 to 9 bits because it is the bit length of the retained count, including reserved row zero. Change `maxUnits` to see the scheduled slot change while the width stays fixed. These are authored calculator inputs, not a recovered catalog for any recording.
{{< /format-demo >}}

{{< callout kind="warning" title="Bytes need a matching catalog and runtime context" >}}
`maxUnits` is per-player slot capacity. It does not determine definition-index width, unit identity or the ground/air grammar. Some final status fields also depend on actual mover presence. A reader without that context must report ambiguity rather than infer a universal layout.
{{< /callout >}}
...
{{< callout kind="established" title="Resource snapshots overwrite state" >}}
The `28` prefix stores a request flag and score counters, not owner IDs. Requested resource totals are not necessarily paid work, and the last float is not an income rate. Ownership comes from transport/session identity. The `28`/`29` exchange coordinates results snapshots; it is not rollback or a checksum comparison.
{{< /callout >}}
...
{{< callout kind="note" title="Three clocks, with different jobs" >}}
`dtMs` measures capture time, `2C` ticks track a sender’s simulation counter, and `19` records change speed. A non-sync event bracketed by neighboring ticks has an interval of capture-order evidence, not an exact recovered simulation tick. Clock gaps and speed changes must remain visible in an analysis.
{{< /callout >}}
...
This page adapts the [owning research]({{< research >}}) at the pinned engine revision. [Download the complete source snapshot](research-source.txt). Bracketed evidence IDs refer to the owning research ledger; observations and implementation policy retain their original scope.
