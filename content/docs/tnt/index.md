+++
title = 'Terrain, one layer at a time'
seoTitle = 'TNT file format — illustrated Total Annihilation reference'
description = 'Explore the tile art, corner heights, feature footprints and minimap that turn a compact binary file into a Total Annihilation landscape.'
type = 'format'
url = '/docs/formats/tnt/'
format = 'TNT'
extension = '.tnt'
...
## Four layers make a map {#overview}

**Established** `[02 R-MAP-01 §§6–8]`, `[03 R-TERR-01]`.
The `.tnt` file is the binary half of a map (paired with an `.ota`,
[ota.md]({{< research "research/formats/ota.md" >}})). It contains the terrain image as a grid of 32×32-pixel
...
([pal.md]({{< research "research/formats/pal.md" >}})).
...
## Peel back the terrain {#terrain-example}

{{< format-demo name="tnt-layers" id="terrain-example-demo" title="One map, two grids, four layers" label="Inspect a TNT" >}}
An original island decoded from [example.tnt](example.tnt), with [inspection data](example.json). Switch between the repeated tile art, corner heights and feature words. Overlay either grid to see that **four attribute cells fit under one tile**. Move the water threshold to inspect `height < SeaLevel` without changing the downloaded file.
{{< /format-demo >}}

{{< callout kind="note" title="A decode example, with an original display palette" >}}
This 512 × 384-pixel terrain, palette and diagrams were authored for this reference. The PNG-free SVG art comes from the fixture’s 32 × 32 indexed tiles; color choices belong to this illustration because TNT stores no palette. Heights and feature views come from the same decoded bytes. The named `ExampleBeacon` has no external feature catalog here, so this is a decoding fixture, not a ready-to-play map. `scripts/tnt-example.py --check` verifies the binary and its generated views.
{{< /callout >}}

| Fixture layer | Stored count | Bytes | What it contributes |
| --- | --- | --- | --- |
| Tile index map | 16 × 12 `u16` values | 384 | A tile choice for each 32-pixel square. |
| Attributes | 32 × 24 four-byte records | 3,072 | Heights, source feature words and the preserved fourth byte. |
| Tile graphics | Four 32 × 32 indexed images | 4,096 | Reusable terrain pixels. |
| Feature table | One 132-byte record | 132 | The original name `ExampleBeacon`. |
| Minimap | 64 × 64 indexes + dimensions | 4,104 | A stored preview with an intentionally padded lower area. |
...
| ---: | ---: | --- | --- | --- |
| 0x00 | 4 | u32 | IDVersion | `0x2000` |
| 0x04 | 4 | u32 | Width | Map width in 16-pixel units (even in observed files) |
| 0x08 | 4 | u32 | Height | Map height in 16-pixel units (even in observed files) |
| 0x0C | 4 | u32 | PtrMapData | → tile index map |
| 0x10 | 4 | u32 | PtrMapAttr | → attribute map |
| 0x14 | 4 | u32 | PtrTileGfx | → tile graphics |
| 0x18 | 4 | u32 | Tiles | Number of unique 32×32 tiles |
| 0x1C | 4 | u32 | TileAnims | Number of feature records (the historical field name is "tile anims"; the records are feature names) |
| 0x20 | 4 | u32 | PtrTileAnims | → feature records |
| 0x24 | 4 | u32 | SeaLevel | Water level in height units; cells with height below this are underwater ("waterheight") |
| 0x28 | 4 | u32 | PtrMiniMap | → minimap |
| 0x2C | 4 | u32 | MinimapPresent | bit 0 set = an embedded minimap follows at PtrMiniMap; `1` in every observed retail map. (Engine reading Established, `[03 R-TERR-01 §1]`; community notes call the word "unknown1".) |
| 0x30–0x3C | 16 | u32×4 | unknown/pad | `0` in observed retail maps |
...
{{< callout kind="warning" title="Version changes the tail and the cells" >}}
The legacy version changes both the header tail and the attribute stride. A canonical cell is **4 bytes**; a legacy cell is **8 bytes**, with its feature byte at +2. SCT sections have yet another header and use tile-grid dimensions. Dispatch by format and version before following pointers.
{{< /callout >}}
...
| Offset | Bytes | Type | Field | Meaning |
| ---: | ---: | --- | --- | --- |
| 0x28 | 4 | u32 | MinWindSpeed | used directly as the map's minimum wind |
| 0x2C | 4 | u32 | MaxWindSpeed | used directly as the maximum wind |
| 0x30 | 4 | u32 | — | not read |
| 0x34 | 4 | u32 | Gravity | authored-unit gravity (same scale as the OTA key); `0` means "use the engine default" |
| 0x38 | 4 | u32 | PtrMiniMap | → minimap |
| 0x3C | 4 | u32 | MinimapPresent | bit 0 |
...
{{< figure src="footprint.svg" alt="A source 4 by 4 feature grid compared to reconstructed occupancy: covered empty words become fringe while an uncovered fringe word becomes empty" width="840" height="380" >}}
Original schematic, independent of the downloadable map. A definition with a 2 × 2 footprint supplies the coverage. The source’s anchor starts the placement; its stored fringe words are not copied into runtime occupancy.
{{< /figure >}}

{{< callout kind="established" title="Reconstruct occupancy; preserve source words" >}}
Stamp authored void first, then live feature anchors in row-major order. Covered `FFFF` becomes fringe; uncovered `FFFE` becomes empty. Keep the raw words available for inspection. The feature definition supplies footprint dimensions.
{{< /callout >}}
...
{{< callout kind="warning" title="Crop the used image before resizing" >}}
A stored minimap’s width and height describe its allocation. They do not identify the used image rectangle. The crop formula above uses playable extents, **Width × 16 − 32** and **Height × 16 − 128**, with the stored bitmap size. Searching for a universal padding color or assuming 252 × 252 can give the wrong picture.
{{< /callout >}}
...
- The file carries no palette; pixels index the global palette ([pal.md]({{< research "research/formats/pal.md" >}})).
